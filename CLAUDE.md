# CLAUDE.md — ClaudeUsage for macOS
...
├── Tools/claude-usage-statusline.sh
├── .gitignore
└── CLAUDE.md
```
...
## Fluxo de dados
`Claude Code -> statusLine stdin -> shell -> ~/Library/Application Support/ClaudeUsage/status/latest.json -> StatusFileMonitor -> ClaudeStatusPayload DTO -> ClaudeStatusMapper -> UsageSnapshot -> SharedUsageStore/App Group -> MenuBarViewModel + Widget TimelineProvider`.

Após persistir snapshot válido, chamar `WidgetCenter.shared.reloadTimelines(ofKind:)`.

## Script statusline
`Tools/claude-usage-statusline.sh` deve:
...
- preservar `~/.claude/settings.json`: alterar somente `statusLine`, nunca substituir o arquivo inteiro.
...
1. Sem arquivo: “Abra o Claude Code e envie uma mensagem para carregar o uso”.
...
Testes não dependem de Claude ativo, rede ou relógio real. Injetar `Date`/clock onde “agora” influenciar regra.
...
1. Validar `xcodebuild -version`, `swift --version`, `claude --version` e estado do projeto/Git.
...
- nova resposta do Claude Code atualiza a fonte local e o app a processa sem scraping.
