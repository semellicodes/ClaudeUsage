...
| MÉDIO, resolvido posteriormente | TestAction vazio | O scheme principal passou a usar `ClaudeUsage.xctestplan`, com o target original `ClaudeUsageCoreTests`. O pacote foi incluído no navegador do projeto para que o Xcode descubra os testes e recursos, além do produto usado pelo app. `Cmd+U` e `xcodebuild test` agora executam os testes do pacote. |
